...
3. Confirm that docs match the current code and call out any stale references.
4. Run lightweight verification where possible.
5. Record known runtime or file-layout gaps instead of burying them.

### Why This Matters
The current repo snapshot is small enough that documentation quality is part of operational safety. If the docs do not explain the exact mutation and migration rules, reviewers can easily approve changes without understanding the governance model. If the docs do not call out stale test references, reviewers can misread "some tests pass" as "the validator is fully verified."
