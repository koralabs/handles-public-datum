...
### Continuation Output
The validator expects a continuation output that holds the target public datum asset and an inline datum that decodes into `PublicDatum`. The source locates that output using `LBL_001 + handle`, which means the continuation asset is part of the state-identity check rather than just an incidental token balance.
...
### Verification Scope
The datum model is now exercised by repo-native transaction scenarios, but reviewers should still distinguish between "the model is implemented clearly" and "every ecosystem integration has been tested." The remaining gap is breadth of coverage, not unresolved file or constant drift.
