...
The technical design is intentionally compact. Most of the spec value is therefore in making the compact source legible: what each helper loads, what each assertion protects, and what the current verification surface does and does not prove.
...
- [tests/fixtures.ts](../../tests/fixtures.ts): repo-native transaction fixtures for owner, dApp, and admin signer scenarios.
- [tests/tests.ts](../../tests/tests.ts): contract scenario harness that exercises `DAPP_UPDATE`, `OWNER_UPDATE`, and `MIGRATE` against `contact.helios`.
...
The settings reference input is located by checking for exactly one unit of that asset class. `LBL_001` is used to locate the public-datum continuation asset, while `LBL_222` identifies both the owner-handle reference token and the `pd_settings` settings handle.
...
`npm test` now runs the repo-aligned validator scenario harness plus the local utility unit tests. The contract scenarios currently prove:

- dApp signers may update only their own datum keys,
- dApps cannot modify `settings`,
- owners may prune entries but cannot rewrite preserved values,
- migration requires an admin signer and conditionally an owner signer.

`tests/contractUtils.test.js` independently covers the helper utility module, and `test_coverage.report` shows 100% measured line and branch coverage for `contractUtils.js`.

### What Remains Out of Scope
The verification story is materially better than the stale snapshot this documentation work replaced, but it is still not exhaustive:

- Node's built-in coverage measures only the JavaScript helper layer, not Helios validator branches.
- The scenario suite exercises representative transactions, not every possible datum shape or integration path.
- The repo does not contain the off-chain builders that produce real production transactions, so those integrations must be reviewed in sibling repos.
